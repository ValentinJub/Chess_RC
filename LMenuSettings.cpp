...
void LMenu::initMenuStrings() {
    std::ifstream settingsLeft("settings.left");
    bool success = true;
    ...
    std::ifstream settingsRight("settings.right");
    success = true;
...
void LMenu::initFont() {
    mFont = TTF_OpenFont("valentin.ttf", 28);
    if(mFont == NULL) {
...
void LMenu::saveSettingsToFile() {
    std::ofstream settings;
        settings.open("settings.config", std::ios::trunc);
    if(settings.is_open()) {
        for(int i(0); i < TOTAL_CLICKABLE_ITEMS - 1; i++) {
